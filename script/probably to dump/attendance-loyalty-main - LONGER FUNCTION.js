...
/*function getLoyalVotes(array, percLoyalVotes) {
    var sumVotes = 0;  
    for (var i = 0; i < array.length; i++){
        ...
        sumVotes = sumVotes + indArray[i].votes_with_party_pct;
        percInd.push(array[i].votes_with_party_pct)
        }
    var avgLoyalty = Math.round(sumVotes/array.length)
    statistics.percLoyalVotes = avgLoyalty+"%"
      }
   } */ 
...
function sortVotes(array){
    ...
    for (var k = 0 ; k < array.lenght; k ++) {
        var partyId = membersTotalNumber[k].party
        if (partyId == "D") {
            statistics.percDemLoyalVotes = avgLoyalty
        }
        if (partyId == "R") {
